...
export default function ChatWidget({ ticker }: Props) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!text || streaming || !ticker) return;
    setInput("");
    send(text, ticker);
  };
        ...
        <div className="mb-3 w-96 h-[520px] bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-neutral-800 bg-neutral-900/95">
            <div>
              ...
              {ticker && (
                <div className="text-xs text-neutral-500 truncate max-w-[240px]">{ticker}</div>
              )}
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
            {!ticker && (
              <div className="text-sm text-neutral-500 text-center py-8">
                View a report first to chat about it.
              </div>
            )}

            {ticker && messages.length === 0 && !streaming && (
              <div className="text-center py-6 space-y-3">
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <button
                      key={q}
                      onClick={() => { setInput(""); send(q, ticker); }}
          ...
          <form onSubmit={handleSubmit} className="p-3 border-t border-neutral-800">
            <div className="flex gap-2">
              <input
                ...
                placeholder={ticker ? "Ask about this analysis..." : "Select a market first"}
              ...
              <button
                type="submit"
                disabled={!input.trim() || streaming || !ticker}
