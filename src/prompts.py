"""Shared prompt templates for the multi-agent analysis pipeline.

Used by both the webapp backend (pipeline.py) and the CLI skill (analyze-market.md).
Each function returns a fully interpolated prompt string ready to send to Claude.
"""

FETCHER_DOCS = """AVAILABLE RESEARCH TOOLS (callable via Bash — use when clearly relevant):
...
def evidence_binary(title: str, resolution_criteria: str, close_date: str, yes_price: str) -> str:
    return f"""SEARCH POLICY: Web search is your primary research tool. Perform up to 7 searches — use them all if the market warrants it. Prioritize breadth early (different angles) then depth on the most important threads.
...
def evidence_event(title: str, close_date: str, resolution_criteria: str, outcomes_text: str) -> str:
    return f"""SEARCH POLICY: Web search is your primary research tool. Perform up to 7 searches — use them all if the market warrants it. Prioritize breadth early (different angles) then depth on the most important threads.
...
def devils_advocate_binary(
    title: str, resolution_criteria: str, close_date: str, yes_price: str,
    evidence: str, sources_pool: str,
) -> str:
    return f"""SEARCH POLICY: Web search is your primary research tool. Perform up to 5 searches — focus entirely on finding counterevidence, historical precedents, and contrarian perspectives NOT already covered by the Evidence Agent. Do not re-research what's already in the sources pool.
...
def devils_advocate_event(
    title: str, close_date: str, outcomes_text: str,
    evidence: str, sources_pool: str,
) -> str:
    return f"""SEARCH POLICY: Web search is your primary research tool. Perform up to 5 searches — focus entirely on finding counterevidence and historical precedents NOT already covered by the Evidence Agent.
...
def resolution_binary(
    title: str, resolution_criteria: str, close_date: str,
    evidence: str, da: str,
) -> str:
    return f"""SEARCH POLICY: You may perform at most 1 additional web search — only if you need to verify a specific ambiguity in the resolution criteria.
...
def resolution_event(
    title: str, close_date: str, resolution_criteria: str, outcomes_text: str,
    evidence: str, da: str,
) -> str:
    return f"""SEARCH POLICY: You may perform at most 1 additional web search — only if you need to verify a specific ambiguity in the resolution criteria.
...
def chaos_binary(
    ...
    return f"""SEARCH POLICY: You may perform at most 1 additional web search — only if you need to research a specific tail risk scenario.
...
def chaos_event(
    title: str, close_date: str, outcomes_text: str, sub_text: str,
    evidence: str, da: str,
) -> str:
    return f"""SEARCH POLICY: You may perform at most 1 additional web search — only if you need to research a specific tail risk scenario.
...
def calibrator_binary(
    title: str, resolution_criteria: str, close_date: str,
    yes_price: str, volume: int,
    evidence: str, da: str, resolution: str, chaos: str,
) -> str:
    return f"""SEARCH POLICY: Do not perform additional web searches. Synthesize only from the provided analyst outputs.
